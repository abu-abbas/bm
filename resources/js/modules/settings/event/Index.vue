...
const table = ref({
  ...
  fields: [
    ...
    },
    {
      key: 'event_name',
      label: 'Nama Event',
      thStyle: { textAlign: 'center', width: '16.6%' }
    },
    {
      key: 'event_header',
      label: 'Judul Event',
...
const onHandleDeleted = item => {
  _confirm(
    {
      title: 'Hapus Event',
      text: `Apakah Anda yakin untuk menghapus data event . ${ item.event_name }?`,
      icon: 'question'
    },
    () => _http.post(
      _route('backend.event.drop'),
      {
        '_method': 'delete',
        id: item.id
      }
    ...
    .then(({ value, isConfirmed, isDismissed }) => {
      ...
      if (isConfirmed && value)
        _alert.fire({ title: `Hapus Event Berhasil`, text: `Event ${item.event_name} berhasil dihapus`, 'icon': 'success' })
...
<template>
  <div class="section-body">
    <h2 class="section-title fs-lg">Daftar Event</h2>
    <p class="section-lead">
      Daftar event yang sudah diinput
    </p>
